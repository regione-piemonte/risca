...
public class AnnualitaSdTributiDAOImpl extends RiscaBeSrvGenericDAO<AnnualitaSdDTO> {
	...
	public AnnualitaSdDTO insertAnnualitaSd(AnnualitaSdDTO dto) throws Exception {
		...
		try {
			...
			LOGGER.error("[AnnualitaUsoSdDAOImpl::insertAnnualitaSd] ERROR : " ,e);
	...
	public AnnualitaSdDTO updateAnnualitaSd(AnnualitaSdDTO dto) throws Exception {
		...
		try {
			...
			LOGGER.error("[AnnualitaUsoSdDAOImpl::updateAnnualitaSd] ERROR : " ,e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			throw new Exception(Constants.ERRORE_GENERICO);
		} catch (SQLException e) {
			LOGGER.error("[AnnualitaUsoSdDAOImpl::updateAnnualitaSd] ERROR : " ,e);
			...
			try {
				...
				for (AnnualitaUsoSdDTO annualitaUsoSdDTO : annualitaSdDTO.getAnnualitaUsoSd()) {
					try {
						...
						LOGGER.error("[AnnualitaUsoSdDAOImpl::deleteAnnualitaSd] ERROR : " ,e);
						TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
						throw new Exception(Constants.ERRORE_GENERICO);
					} catch (Exception e) {
						LOGGER.error("[AnnualitaUsoSdDAOImpl::deleteAnnualitaSd] ERROR : " ,e);
