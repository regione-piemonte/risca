...
import it.csi.risca.riscabesrv.business.be.SoggettoDelegaApi;
...
/**
 * The SoggettoDelegaApiServiceImpl.
 *
 * @author CSI PIEMONTE
 */
@Component
public class SoggettoDelegaApiServiceImpl  extends BaseApiServiceImpl implements SoggettoDelegaApi {
	...
	@Override
	public Response loadSoggettiDelegaByIdDelegato(String fruitore, Long idDelegato, HttpHeaders httpHeaders,
			HttpServletRequest httpRequest) {
		String methodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		LOGGER.debug(getClassFunctionBeginInfo(className, methodName));
		List<SoggettoDelegaExtendedDTO> listSoggettoDelega = new ArrayList<>();
		try {
			identitaDigitaleManager.verificaIdentitaDigitale(fruitore, null, httpHeaders, null);
			listSoggettoDelega = businessLogic.loadSoggettiDelegaByIdDelegato(idDelegato);
		...
		return Response.ok(listSoggettoDelega).header(HttpHeaders.CONTENT_ENCODING, IDENTITY).build();
	}
