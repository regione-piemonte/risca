...
public class RiscossioneAmbienteDAOImpl extends RiscaBeSrvGenericDAO<RiscossioneDTO> {
...
//	private static final String QUERY_MAX_PROG_UTENZA = "SELECT MAX(COD_RISCOSSIONE_PROG) FROM risca_t_riscossione rtr where rtr.cod_riscossione_prov = :codRiscossioneProv"
//					+ " and rtr.id_tipo_riscossione in ( :idTipoRiscossione)";
	...
	private static final String QUERY_SEARCH_RISCOSSIONI_BY_SD = "select distinct rtsd.id_riscossione from RISCA_T_STATO_DEBITORIO rtsd "
			+"inner join risca_t_riscossione rtr on rtr.id_riscossione = rtsd.id_riscossione "+ QUERY_JOIN_SD;
	...
	private static final String QUERY_GROUP_BY_SEARCH_RISCOSSIONI ="  group by rtr.cod_riscossione, rtr.id_riscossione, rtr.num_pratica , rdta.des_tipo_autorizza, rdsr.des_stato_riscossione  "
			+ "  ,rtr.json_dt "
			+ "  , rdts.cod_tipo_soggetto, rtgs.id_gruppo_soggetto,rts.den_soggetto,rts.cognome, "
			+ "   rts.nome,"
			+ "   rdta.id_ambito,"
			+ "   rtr.data_scad_concessione";
	...
	private static final String QUERY_UPDATE_RISCOSSIONE_STATO_PRATICA_SCADUTA = "UPDATE risca_t_riscossione r "
			+ " set id_stato_riscossione =  "+Constants.DB.STATO_RISCOSSIONE.ID_STATO_RISCOSSIONE.SCADUTA + ",  gest_data_upd= :gestDataUpd, gest_attore_upd= :gestAttoreUpd where id_stato_riscossione = "+Constants.DB.STATO_RISCOSSIONE.ID_STATO_RISCOSSIONE.ATTIVA
			+ " and data_scad_concessione < current_date  and NOT EXISTS (select * from risca_r_provvedimento p  where r.id_riscossione = p.id_riscossione and id_tipo_provvedimento IN ("+Constants.DB.TIPO_PROVVEDIMENTO.ID_TIPO_PROVVEDIMENTO.IST_RINNOVO+ ", "+Constants.DB.TIPO_PROVVEDIMENTO.ID_TIPO_PROVVEDIMENTO.IST_SANATORIA+", "+Constants.DB.TIPO_PROVVEDIMENTO.ID_TIPO_PROVVEDIMENTO.AUT_PROVVISORIA+") ) ";

	private static final String QUERY_UPDATE_RISCOSSIONE_STATO_PRATICA_SOSPESA = "update risca_t_riscossione  "
			+ " set id_stato_riscossione =   "+Constants.DB.STATO_RISCOSSIONE.ID_STATO_RISCOSSIONE.SOSPESA + " ,  gest_data_upd= :gestDataUpd, gest_attore_upd= :gestAttoreUpd where id_stato_riscossione = "+Constants.DB.STATO_RISCOSSIONE.ID_STATO_RISCOSSIONE.ATTIVA
			+ " and current_date >= data_ini_sosp_canone and current_date <= data_fine_sosp_canone ";

	private static final String QUERY_UPDATE_RISCOSSIONE_STATO_PRATICA_ATTIVA = "update risca_t_riscossione  "
			+ " set id_stato_riscossione =   "+Constants.DB.STATO_RISCOSSIONE.ID_STATO_RISCOSSIONE.ATTIVA + " ,  gest_data_upd= :gestDataUpd, gest_attore_upd= :gestAttoreUpd where id_stato_riscossione = "+Constants.DB.STATO_RISCOSSIONE.ID_STATO_RISCOSSIONE.SOSPESA
			+ " and current_date > data_fine_sosp_canone ";
	...
	public Long saveRiscossione(RiscossioneDTO dto, String fruitore) throws Exception{
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::saveRiscossione] BEGIN");
...
//DP sposto if all'interno del ciclo per settare idComune
				...
				for(int i = 0; i < dto.getSoggetto().getRecapiti().size(); i++) {				
		            if(dto.getSoggetto().getRecapiti().get(i).getComuneRecapito().getIdComune() != null) {
		            	map.put("idComune", dto.getSoggetto().getRecapiti().get(i).getComuneRecapito().getIdComune());            	
			            	...
			            	if(fruitore == null) {
				            	ComuneExtendedDTO comuneValido = template.queryForObject(QUERY_LOAD_COMUNE, params, getComuneRowMapper());
					            if(comuneValido.getDataFineValidita() != null) {
					            	...
					            	throw new BusinessException(409, "E085", "Attenzione: il comune di uno o piu' recapiti non e' piu' valido. Prima di procedere e' necessario selezionare un comune valido.o.");
			...
			if (dto.getCodRiscossioneProg() != null) {
				if(dto.getCodRiscossioneProg().isEmpty()) {
			       	messaggiDAO.loadMessaggiByCodMessaggio("E085");
					throw new BusinessException(400, "E085", "Attenzione il codice progressivo utenza non e' stato specificato correttamente. Se non si vuole specificare non mettere il tag nel json.", null);

				}
				if (idAmbito.equals(1L)) {
					if(fruitore == null || (fruitore != null && !fruitore.equals(Constants.BATCH_PORTING)) ) {
		                if(Integer.parseInt(dto.getCodRiscossioneProg()) > 9999 &&  dto.getIdTipoRiscossione() == 1) {
		            		...
		            		throw new BusinessException(400, "E019", "Attenzione il codice progressivo utenza per le pratiche Ordinarie deve essere compreso tra 1 e 9.999.", null);
		                }
		                
		                if((Integer.parseInt(dto.getCodRiscossioneProg()) < 10000 || Integer.parseInt(dto.getCodRiscossioneProg()) > 49999) &&  dto.getIdTipoRiscossione() == 2) {
		                	messaggiDAO.loadMessaggiByCodMessaggio("E020");
		                	throw new BusinessException(400, "E020", "Attenzione il codice progressivo utenza per le pratiche Preferenziale deve essere compreso tra 10.000 e 49.999", null);
		                }
						
		                if(Integer.parseInt(dto.getCodRiscossioneProg()) < 50000 &&  (dto.getIdTipoRiscossione() == 3 || dto.getIdTipoRiscossione() == 4) ) {
		                	messaggiDAO.loadMessaggiByCodMessaggio("E021");
		                	throw new BusinessException(400, "E021", "Attenzione il codice progressivo utenza per gli Attingimenti deve essere maggiore o uguale a 50.000", null);
		                }
		                	
					}
					
	                String codRiscProg = dto.getCodRiscossioneProg();
					switch (codRiscProg.length()) {
					case 1:
						codRiscProg = "0000" + codRiscProg;
						break;
					case 2:
						codRiscProg = "000" + codRiscProg;
						break;
					case 3:
						codRiscProg = "00" + codRiscProg;
						break;
					case 4:
						codRiscProg = "0" + codRiscProg;
						break;
					default:

					}
	                
					map.put("codRiscossione", dto.getCodRiscossioneProv() + codRiscProg);
				}
                ...
                if(Integer.parseInt(codRiscoss) > 9999 &&  dto.getIdTipoRiscossione() == 1) {
            		...
            		throw new BusinessException(400, "E045", "Attenzione: per il tipo di riscossione indicato e stato raggiunto il limite previsto per il valore del progressivo utenza. Non e possibile salvare la riscossione.", null);

                }
                
                
                if((Integer.parseInt(codRiscoss) < 10000 || Integer.parseInt(codRiscoss) > 49999) &&  dto.getIdTipoRiscossione() == 2) 
                {
                 	messaggiDAO.loadMessaggiByCodMessaggio("E045");
            		throw new BusinessException(400, "E045", "Attenzione: per il tipo di riscossione indicato e stato raggiunto il limite previsto per il valore del progressivo utenza. Non e possibile salvare la riscossione.", null);
                ...
                if(Integer.parseInt(codRiscoss) < 50000 &&  (dto.getIdTipoRiscossione() == 3 || dto.getIdTipoRiscossione() == 4) ) 
                {
                 	messaggiDAO.loadMessaggiByCodMessaggio("E045");
            		throw new BusinessException(400, "E045", "Attenzione: per il tipo di riscossione indicato e stato raggiunto il limite previsto per il valore del progressivo utenza. Non e possibile salvare la riscossione.", null);
			...
			try {
				...
				if (dataScadConcessione != null) {
					if (!dataScadConcessione.isEmpty()) {
				...
				if (dataRinunciaRevocaST != null) {
					if (!dataRinunciaRevocaST.isEmpty()) {
				  ...
				  LOGGER.error("[RiscossioneAmbienteDAOImpl::saveRiscossione] Errore nel mapping dell'oggetto ", e);
			}
...
//            String uid = generateGestUID(dto.getCfAccredito() + dto.getDesEmailAccredito() + now);
			...
			for (int i = 0; i < sizeArrayProvv; i++) {
				...
				map.put("idTipoTitolo", dto.getProvvedimento().get(i).getTipoTitoloExtendedDTO()!= null ? dto.getProvvedimento().get(i).getTipoTitoloExtendedDTO().getIdTipoTitolo() : null);
				map.put("idTipoProvvedimento", dto.getProvvedimento().get(i).getTipiProvvedimentoExtendedDTO().getIdTipoProvvedimento());
			...
			if(!CollectionUtils.isEmpty(dto.getRecapitiRiscossione())) {
			    ...
			    //	}
		        	...
		        	params = getParameterValue(map);
		        	template.update(getQuery(QUERY_INSERT_RISCOSSIONE_RECAPITO, null, null), params, keyHolder, new String[]{"id_riscossione"});
			...
			LOGGER.error("[RiscossioneAmbienteDAOImpl::saveRiscossione] ERROR Business Exception : " + be);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
        	throw be;
        } catch (Exception e) {
			LOGGER.error("[RiscossioneAmbienteDAOImpl::saveRiscossione] ERROR : " + e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			throw e;
		} finally {
			LOGGER.debug("[RiscossioneAmbienteDAOImpl::saveRiscossione] END");
		}
	}

	public String creaWhereConditionSearch(Map<String, Object> map , SimpleDateFormat formatter, RiscossioneSearchDTO dto,
			Long idAmbito,SimpleDateFormat formatter2, boolean filtriSD ) throws Exception {
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::creaWhereConditionSearch] BEGIN");
		String whereCond = "";
		String and = " and ";
		...
		if (dto.getComuneDiResidenza() != null) {
			whereCond = whereCond + and + "    replace (rdc.denom_comune, '''','') = :comuneDiResidenza and rrr.id_tipo_recapito =1  ";
		...
		if (!CollectionUtils.isEmpty(dto.getProvvedimenti())) {
			...
			for (ProvvedimentoSearchDTO provvedimentoSearch : listProvvedimenti) {
				...
				if(provvedimentoSearch.getIdProvvedimento() != null) {
					query = query +  and +   " (rrp.id_tipo_provvedimento  = :idProvvedimento"+indexProvvedimenti+") ";
		...
		if (!CollectionUtils.isEmpty(dto.getIstanze())) {
			...
			for (TipoIstanzaDTO tipoIstanzaDTO : listTipoIstanzaDTO) {
				   String query= "";
				if(indexTipoIstanza > 0) {
						queryTipoIstanza = queryTipoIstanza  + " OR ";
					}
					query = query + "( ( rdtp.flg_istanza = 1 )";
				if(tipoIstanzaDTO.getIdIstanza() != null) {
					query = query +  and +   " (rrp.id_tipo_provvedimento  = :idIstanza"+indexTipoIstanza+")";
					map.put("idIstanza"+indexTipoIstanza, tipoIstanzaDTO.getIdIstanza());
				}
				if (!StringUtils.isEmpty(tipoIstanzaDTO.getDataIstanzaDa()) && StringUtils.isEmpty(tipoIstanzaDTO.getDataIstanzaA()) ) {
					query = query +  and + "( rrp.data_provvedimento  >= :dataIstanzaDa"+indexTipoIstanza+" ) ";
					Date dataIstanzaDa = formatter2.parse(formatter2.format(formatter.parse(tipoIstanzaDTO.getDataIstanzaDa())));
					map.put("dataIstanzaDa"+indexTipoIstanza, dataIstanzaDa);
				}
				if (StringUtils.isEmpty(tipoIstanzaDTO.getDataIstanzaDa()) && !StringUtils.isEmpty(tipoIstanzaDTO.getDataIstanzaA()) )  {
					query = query +  and + " (rrp.data_provvedimento  <= :dataIstanzaA"+indexTipoIstanza+") ";
					Date dataIstanzaA = formatter2.parse(formatter2.format(formatter.parse(tipoIstanzaDTO.getDataIstanzaA())));
					map.put("dataIstanzaA"+indexTipoIstanza, dataIstanzaA);
				}
				if (!StringUtils.isEmpty(tipoIstanzaDTO.getDataIstanzaDa()) && !StringUtils.isEmpty(tipoIstanzaDTO.getDataIstanzaA()) )  {
					query = query + and
							+ " (rrp.data_provvedimento  BETWEEN :dataIstanzaDa"+indexTipoIstanza+" and :dataIstanzaA"+indexTipoIstanza+") ";
					Date dataIstanzaA = formatter2.parse(formatter2.format(formatter.parse(tipoIstanzaDTO.getDataIstanzaA())));
					Date dataIstanzaDa =  formatter2.parse(formatter2.format(formatter.parse(tipoIstanzaDTO.getDataIstanzaDa())));
					map.put("dataIstanzaDa"+indexTipoIstanza, dataIstanzaDa);
					map.put("dataIstanzaA"+indexTipoIstanza, dataIstanzaA);
		...
		whereCond = creaQueryDatiTecnici(dto, whereCond, and, formatter2, formatter,filtriSD);
		
		if(!filtriSD) {
			whereCond = whereCond + QUERY_GROUP_BY_SEARCH_RISCOSSIONI;
		}
		if(indexTipoIstanza > 1 ) {
			whereCond = whereCond + "having count(*) > " + (indexTipoIstanza - 1);
		}
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::creaWhereConditionSearch] END");
		return whereCond;
	}
	
	private String creaQueryDatiGenerali(DatiTecniciAmbienteDTO datiTecniciAmbiente,String whereCond, String and, Boolean filtriSD ) {
		if(!filtriSD) {
			if (datiTecniciAmbiente.getDatiGenerali().getCorpoIdricoCaptazione() != null )
				whereCond = whereCond + and
						+ " upper (replace (replace (CAST( json_dt ->'riscossione'->'dati_tecnici'->'dati_generali'->'corpo_idrico_captazione' as varchar) ,'\"',''),'''', ''))  = '"
						+ datiTecniciAmbiente.getDatiGenerali().getCorpoIdricoCaptazione().replace("'", "").toUpperCase()+"'" ;

			if (datiTecniciAmbiente.getDatiGenerali().getComune() != null )
				whereCond = whereCond + and
						+ " upper (replace (CAST( json_dt ->'riscossione'->'dati_tecnici'->'dati_generali'->'comune' as varchar)  ,'''', ''))= '\""
					+ datiTecniciAmbiente.getDatiGenerali().getComune().replace("'", "").toUpperCase()  + "\"' ";

			if (datiTecniciAmbiente.getDatiGenerali().getNomeImpiantoIdrico() != null )
				whereCond = whereCond + and
						+ "  upper (replace (replace (CAST( json_dt ->'riscossione'->'dati_tecnici'->'dati_generali'->'nome_impianto_idrico' as varchar),'\"',''),'''', '')) = '"
						+ datiTecniciAmbiente.getDatiGenerali().getNomeImpiantoIdrico().replace("'", "").toUpperCase()+"' " ;
		
			if (datiTecniciAmbiente.getDatiGenerali().getPortataDaAssegnare() != null && !datiTecniciAmbiente.getDatiGenerali().getPortataDaAssegnare().equals(BigDecimal.ZERO) )
				whereCond = whereCond + and
						+ " CAST( json_dt ->'riscossione'->'dati_tecnici'->'dati_generali'->'portata_da_assegnare' as numeric) = "
						+ datiTecniciAmbiente.getDatiGenerali().getPortataDaAssegnare();
			
		}else {
			if (datiTecniciAmbiente.getDatiGenerali().getCorpoIdricoCaptazione() != null )
				whereCond = whereCond + and
						+ " ( upper (replace (replace (CAST( rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'dati_generali'->'corpo_idrico_captazione' as varchar) ,'\"',''),'''', ''))  = '"
						+ datiTecniciAmbiente.getDatiGenerali().getCorpoIdricoCaptazione().replace("'", "").toUpperCase()+"' " 
						+ " OR "
						+ " upper (replace (replace (CAST( rras.json_dt_riscossione ->'dati_generali'->'corpo_idrico_captazione' as varchar) ,'\"',''),'''', ''))  = '"
						+ datiTecniciAmbiente.getDatiGenerali().getCorpoIdricoCaptazione().replace("'", "").toUpperCase()+"' "
						+ " ) " ;

			if (datiTecniciAmbiente.getDatiGenerali().getComune() != null )
				whereCond = whereCond + and
						+ "( upper (replace (CAST( rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'dati_generali'->'comune' as varchar)  ,'''', ''))= '\""
						+ datiTecniciAmbiente.getDatiGenerali().getComune().replace("'", "").toUpperCase() + "\"' "
						+ " OR "
						+ " upper (replace (CAST( rras.json_dt_riscossione ->'dati_generali'->'comune' as varchar)  ,'''', ''))= '\""
						+ datiTecniciAmbiente.getDatiGenerali().getComune().replace("'", "").toUpperCase() + "\"' "
						+ " ) ";

			if (datiTecniciAmbiente.getDatiGenerali().getNomeImpiantoIdrico() != null )
				whereCond = whereCond + and
						+ " ( upper (replace (replace (CAST( rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'dati_generali'->'nome_impianto_idrico' as varchar),'\"',''),'''', '')) = '"
						+ datiTecniciAmbiente.getDatiGenerali().getNomeImpiantoIdrico().replace("'", "").toUpperCase()+"' " 
						+ " OR "
						+ "  upper (replace (replace (CAST( rras.json_dt_riscossione ->'dati_generali'->'nome_impianto_idrico' as varchar),'\"',''),'''', '')) = '"
						+ datiTecniciAmbiente.getDatiGenerali().getNomeImpiantoIdrico().replace("'", "").toUpperCase()+"' " 
						+ " ) ";
			if (datiTecniciAmbiente.getDatiGenerali().getPortataDaAssegnare() != null && !datiTecniciAmbiente.getDatiGenerali().getPortataDaAssegnare().equals(BigDecimal.ZERO) )
				whereCond = whereCond + and
						+ " ( CAST( rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'dati_generali'->>'portata_da_assegnare' as numeric ) = "
						+ datiTecniciAmbiente.getDatiGenerali().getPortataDaAssegnare()
						+ " OR "
						+ " CAST( rras.json_dt_riscossione ->'dati_generali'->>'portata_da_assegnare' as numeric) = "
						+ datiTecniciAmbiente.getDatiGenerali().getPortataDaAssegnare()
						+ " ) ";
		}
		return whereCond;
	...
	private String creaQueryUsiDatoTecnico(DatiTecniciAmbienteDTO datiTecniciAmbiente,List<String> listNomiTipoUso, String whereCond, String and, SimpleDateFormat formatter2,SimpleDateFormat formatter, Boolean filtriSD ) throws ParseException{
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::creaQueryDatiTecnici] BEGIN");
		String nometipoUsoDatoTecnico = "";
		int count = 0;
		String queryTipoUso ="";
		String andTipoUso ="";
		String fineAndTipoUso= "";
		for (String keytipoUsoDatoTecnico : datiTecniciAmbiente.getUsi().keySet()) {
			andTipoUso = " and ( ";
			nometipoUsoDatoTecnico = keytipoUsoDatoTecnico;

			String query= "";
			if (!"".equals(nometipoUsoDatoTecnico) && !"undefined".equals(nometipoUsoDatoTecnico)  ) {
				TipoUsoDatoTecnicoDTO TipoUsoDatoTecnicoDTO = datiTecniciAmbiente.getUsi().get(nometipoUsoDatoTecnico);
				listNomiTipoUso.add(TipoUsoDatoTecnicoDTO.getTipoUsoLegge());
				
				if (TipoUsoDatoTecnicoDTO != null) {
					if(count > 0) {
				    ...
				    if(!filtriSD) {
						...
						query = query + "( replace (CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
								...
								+ TipoUsoDatoTecnicoDTO.getTipoUsoLegge().replace("'", "") + "\"'  )";

				        String dataScadenzaEMASoISODa = !StringUtils.isEmpty(TipoUsoDatoTecnicoDTO.getDataScadenzaEmasIsoDa())
				                ? Utils.formatDate(TipoUsoDatoTecnicoDTO.getDataScadenzaEmasIsoDa(), formatter, formatter2)
				                : null;

				        String dataScadenzaEMASoISOA = !StringUtils.isEmpty(TipoUsoDatoTecnicoDTO.getDataScadenzaEmasIsoA())
				                ? Utils.formatDate(TipoUsoDatoTecnicoDTO.getDataScadenzaEmasIsoA(), formatter, formatter2)
				                : null;
						if (dataScadenzaEMASoISODa != null && dataScadenzaEMASoISOA == null)
							query = query + and + "( CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->'data_scadenza_emas_iso' as varchar) >= '\""
									+ dataScadenzaEMASoISODa + "\"')";

						if (dataScadenzaEMASoISODa == null && dataScadenzaEMASoISOA != null)
							query = query + and + "( CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->'data_scadenza_emas_iso' as varchar) <= '\""
									+ dataScadenzaEMASoISOA + "\"')";

						if (dataScadenzaEMASoISODa != null && dataScadenzaEMASoISOA != null)
							query = query + and + "( CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->'data_scadenza_emas_iso' as varchar) BETWEEN '\""
									+ dataScadenzaEMASoISODa + "\"'  AND  '\"" + dataScadenzaEMASoISOA + "\"')";

						long idTipoUsoLegge = TipoUsoDatoTecnicoDTO.getIdTipoUsoLegge() != null
								? TipoUsoDatoTecnicoDTO.getIdTipoUsoLegge()
								: 0;
						if (idTipoUsoLegge > 0)
							query = query + and + " CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
						...
						String[] tipoUsoSpecificotList = TipoUsoDatoTecnicoDTO.getTipoUsoSpecifico() != null
								|| TipoUsoDatoTecnicoDTO.getTipoUsoSpecifico().length > 0
										? TipoUsoDatoTecnicoDTO.getTipoUsoSpecifico()
										: null;
						if (TipoUsoDatoTecnicoDTO.getTipoUsoSpecifico().length > 0) {
							for (int i = 0; i < tipoUsoSpecificotList.length; i++) {
								query = query + and + " ( rtr.id_riscossione in (  "
										+ "   SELECT id_riscossione  "
										+ "   FROM  risca_t_riscossione,  jsonb_array_elements_text(json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
										+ nometipoUsoDatoTecnico + "'->'uso_specifico')  as uso_specifico "
										+ " WHERE uso_specifico = '" + tipoUsoSpecificotList[i] + "') )";
							}

						}
						BigDecimal quantitaDa = TipoUsoDatoTecnicoDTO.getQuantitaDa() != null
								? TipoUsoDatoTecnicoDTO.getQuantitaDa()
								: null;
						BigDecimal quantitaA = TipoUsoDatoTecnicoDTO.getQuantitaA() != null
								? TipoUsoDatoTecnicoDTO.getQuantitaA()
								: null;
						if (!Objects.isNull(quantitaDa) && Objects.isNull(quantitaA))
							query = query + and + " (CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->'qta_acqua'   as Integer) >= " + quantitaDa + ")";

						if (Objects.isNull(quantitaDa) && !Objects.isNull(quantitaA))
							query = query + and + " (CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->'qta_acqua'   as Integer) <= " + quantitaA + ")";

						if (!Objects.isNull(quantitaDa) && !Objects.isNull(quantitaA))
							query = query + and + " (CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->'qta_acqua'   as Integer) BETWEEN " + quantitaDa
									+ " AND " + quantitaA + ")";
						...
						if (idTipoUsoLegge > 0)
							query = query + and + ""
									...
									+ idTipoUsoLegge
									+ " OR "
									+ " CAST( rras.json_dt_riscossione ->'usi'->'"
									+ nometipoUsoDatoTecnico + "' ->'id_tipo_uso_legge' as Integer) = "
									+ idTipoUsoLegge
									+ " ) ";
						...
						if (TipoUsoDatoTecnicoDTO.getTipoUsoSpecifico().length > 0) {
							for (int i = 0; i < tipoUsoSpecificotList.length; i++) {
								query = query + and + " ( rtsd.id_stato_debitorio in (  "
										+ "   SELECT id_stato_debitorio  "
										+ "   FROM  risca_r_annualita_sd,  jsonb_array_elements_text(json_dt_riscossione ->'riscossione'->'dati_tecnici'->'usi'->'"
										+ nometipoUsoDatoTecnico + "'->'uso_specifico')  as uso_specifico "
										+ " WHERE uso_specifico = '" + tipoUsoSpecificotList[i] + "' "
										+ " GROUP by id_stato_debitorio "
										+ " UNION "
										+ " SELECT id_stato_debitorio  "
										+ " FROM  risca_r_annualita_sd,  jsonb_array_elements_text(json_dt_riscossione ->'usi'->'"
										+ nometipoUsoDatoTecnico + "'->'uso_specifico')  as uso_specifico "
										+ " WHERE uso_specifico = '" + tipoUsoSpecificotList[i] + "' "
										+ " GROUP by id_stato_debitorio "
										+ " )"
						...
						if (!Objects.isNull(quantitaDa) && Objects.isNull(quantitaA))
							query = query + and + ""
									+ " ( "
									+ " (CAST( rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->>'qta_acqua'   as numeric) >= " + quantitaDa + ") "
									+ " OR "
									+ " (CAST(  rras.json_dt_riscossione ->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->>'qta_acqua'   as numeric) >= " + quantitaDa + ") "
									+ " ) ";
						...
						if (Objects.isNull(quantitaDa) && !Objects.isNull(quantitaA))
							query = query + and + "" 
						            + " ( "
									+ " (CAST(  rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->>'qta_acqua'   as numeric) <= " + quantitaA + ") "
									+ " OR "
									+ " (CAST(  rras.json_dt_riscossione ->'usi'->'" + nometipoUsoDatoTecnico
									+ "'->>'qta_acqua'   as numeric) <= " + quantitaA + ") "
									+ " ) ";
						...
						if (!Objects.isNull(quantitaDa) && !Objects.isNull(quantitaA))
							query = query + and + "" 
						            + " ( "
									+ " (CAST(  rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->>'qta_acqua'   as numeric) BETWEEN " + quantitaDa
									+ " AND " + quantitaA + ") "
									+ " OR " 
									+ " (CAST(  rras.json_dt_riscossione ->'usi'->'"
									+ nometipoUsoDatoTecnico + "'->>'qta_acqua'   as numeric) BETWEEN " + quantitaDa
			...
			if("undefined".equals(nometipoUsoDatoTecnico)) {
				  List<TipoUsoExtendedDTO> ListTipoUso =	tipoUsoDao.loadTipoUso().stream().filter(t -> t.getIdTipoUsoPadre() == null ).collect(Collectors.toList());
				  TipoUsoDatoTecnicoDTO TipoUsoDatoTecnicoDTO = datiTecniciAmbiente.getUsi().get(nometipoUsoDatoTecnico);
				  for (TipoUsoExtendedDTO nomeTipoUso : ListTipoUso) {
					   query= "";
					    ...
					    if(!filtriSD) {
							...
							if (dataScadenzaEMASoISODa != null && dataScadenzaEMASoISOA == null)
								query = query + and + "( CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
										+ nomeTipoUso.getCodTipouso() + "'->'data_scadenza_emas_iso' as varchar) >= '\""
										+ dataScadenzaEMASoISODa + "\"')";
							...
							if (dataScadenzaEMASoISODa != null && dataScadenzaEMASoISOA != null)
								query = query + and + "( CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
										+ nomeTipoUso.getCodTipouso() + "'->'data_scadenza_emas_iso' as varchar) BETWEEN '\""
										+ dataScadenzaEMASoISODa + "\"'  AND  '\"" + dataScadenzaEMASoISOA + "\"')";
							...
							if (!Objects.isNull(quantitaDa) && Objects.isNull(quantitaA))
								query = query + and + " (CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
										+ nomeTipoUso.getCodTipouso()  + "'->'qta_acqua'   as Integer) >= " + quantitaDa+")";
							
							
							if (Objects.isNull(quantitaDa) && !Objects.isNull(quantitaA))
								query = query + and + " (CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
										+ nomeTipoUso.getCodTipouso()  + "'->'qta_acqua'   as Integer) <= " + quantitaA +")";
							
							if (!Objects.isNull(quantitaDa) && !Objects.isNull(quantitaA))
									query = query + and + " (CAST( json_dt ->'riscossione'->'dati_tecnici'->'usi'->'"
											+ nomeTipoUso.getCodTipouso()  + "'->'qta_acqua'   as Integer) BETWEEN " + quantitaDa
											+ " AND " + quantitaA +")";
							...
							query = query + "( "
									+ " ( replace (CAST(  rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'usi'->'"
									+ nomeTipoUso.getCodTipouso() + "' ->'uso_di_legge' as varchar),'''', '') = '\""
									+ nomeTipoUso.getCodTipouso().replace("'", "") + "\"'  ) " 
									+ " OR "
									+ " ( replace (CAST(  rras.json_dt_riscossione ->'usi'->'"
									+ nomeTipoUso.getCodTipouso() + "' ->'uso_di_legge' as varchar),'''', '') = '\""
									+ nomeTipoUso.getCodTipouso().replace("'", "") + "\"'  ) "
							...
							if (idTipoUsoLegge > 0)
								query = query + and + ""
										+ " ( "
										+ " CAST(  rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'usi'->'"
										+ nomeTipoUso.getCodTipouso() + "' ->'id_tipo_uso_legge' as Integer) = "
										+ idTipoUsoLegge
										+ " OR "
										+ " CAST(  rras.json_dt_riscossione ->'usi'->'"
										+ nomeTipoUso.getCodTipouso() + "' ->'id_tipo_uso_legge' as Integer) = "
							...
							if (TipoUsoDatoTecnicoDTO.getTipoUsoSpecifico().length > 0) {
								for (int i = 0; i < tipoUsoSpecificotList.length; i++) {
									query = query + and + " ( rtsd.id_stato_debitorio in (  "
											+ "   SELECT id_stato_debitorio  "
											+ "   FROM  risca_r_annualita_sd,  jsonb_array_elements_text(json_dt_riscossione ->'riscossione'->'dati_tecnici'->'usi'->'"
											+ nomeTipoUso.getCodTipouso() + "'->'uso_specifico')  as uso_specifico "
											+ " WHERE uso_specifico = '" + tipoUsoSpecificotList[i] + "' "
											+ " GROUP by id_stato_debitorio "
											+ " UNION "
											+ " SELECT id_stato_debitorio  "
							...
							if (!Objects.isNull(quantitaDa) && Objects.isNull(quantitaA))
								query = query + and + ""
										+ " ( "
										+ " (CAST(  rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'usi'->'"
										+ nomeTipoUso.getCodTipouso() + "'->>'qta_acqua'   as numeric) >= " + quantitaDa + ") "
										+ " OR "
										+ " (CAST(  rras.json_dt_riscossione ->'usi'->'"
										+ nomeTipoUso.getCodTipouso() + "'->>'qta_acqua'   as numeric) >= " + quantitaDa + ") "
										+ " ) ";
							...
							if (Objects.isNull(quantitaDa) && !Objects.isNull(quantitaA))
								query = query + and + "" 
							            + " ( "
										+ " (CAST(  rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'usi'->'"
										+ nomeTipoUso.getCodTipouso() + "'->>'qta_acqua'   as numeric) <= " + quantitaA + ") "
										+ " OR "
										+ " (CAST(  rras.json_dt_riscossione ->'usi'->'" + nomeTipoUso.getCodTipouso()
										+ "'->>'qta_acqua'   as numeric) <= " + quantitaA + ") "
										+ " ) ";
							...
							if (!Objects.isNull(quantitaDa) && !Objects.isNull(quantitaA))
								query = query + and + "" 
							            + " ( "
										+ " (CAST(  rras.json_dt_riscossione ->'riscossione'->'dati_tecnici'->'usi'->'"
										+ nomeTipoUso.getCodTipouso() + "'->>'qta_acqua'   as numeric) BETWEEN " + quantitaDa
										+ " AND " + quantitaA + ") "
										+ " OR " 
										+ " (CAST(  rras.json_dt_riscossione ->'usi'->'"
										+ nomeTipoUso.getCodTipouso() + "'->>'qta_acqua'   as numeric) BETWEEN " + quantitaDa
		...
		return whereCond = whereCond + andTipoUso + queryTipoUso +fineAndTipoUso;
		
	}
	private String creaQueryDatiTecnici( RiscossioneSearchDTO dto, String whereCond, String and,SimpleDateFormat formatter2,SimpleDateFormat formatter, Boolean filtriSD) throws JsonParseException, JsonMappingException, IOException, ParseException {
		if (dto.getDatiTecnici() != null) {
			LOGGER.debug("[RiscossioneAmbienteDAOImpl::creaQueryDatiTecnici] BEGIN");
			ObjectMapper mapper = new ObjectMapper().configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);;
			List<String> listNomiTipoUso = new ArrayList<>();  
			// parte della ricerca Dati Tecnici
			DatiTecniciAmbienteDTO datiTecniciAmbiente = mapper.readValue(dto.getDatiTecnici(), DatiTecniciAmbienteDTO.class); // recupero dati tecnici dal campo json_dt
			if(datiTecniciAmbiente != null) {
				if (datiTecniciAmbiente.getDatiGenerali() != null) {
					whereCond = creaQueryDatiGenerali(datiTecniciAmbiente, whereCond, and, filtriSD);
				}
				// parte della ricerca Uso Dato Tecnico
				whereCond =  creaQueryUsiDatoTecnico(datiTecniciAmbiente, listNomiTipoUso, whereCond, and, formatter2, formatter, filtriSD);
			}
		}
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::creaQueryDatiTecnici] END");
		return whereCond;
	...
	public List<RiscossioneSearchResultDTO> searchRiscossioni(RiscossioneSearchDTO dto, Long idAmbito,String modalitaRicerca , Integer offset, Integer limit, String sort) throws Exception {
		...
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::searchRiscossione] BEGIN");
		try {
			Map<String, Object> map = new HashMap<>();
...
//					crea query per lo SD 
					 ...
					 String querySd = " where  rtr.id_riscossione in ( "+  addJoinQueryFromDto(dto, modalitaRicerca, QUERY_SEARCH_RISCOSSIONI_BY_SD)
		    ...
		    riscossioneSearchResultDTO = template.query(getQuery(queySearch + whereCond+ dynamicOrderByCondition, offset!= null ? offset.toString(): null ,  limit != null ? limit.toString(): null), params,
					getRowMapperRiscossioneSearch());

			return riscossioneSearchResultDTO;
		} catch (EmptyResultDataAccessException e) {
			LOGGER.debug("[RiscossioneAmbienteDAOImpl::searchRiscossione] Data not found");
			return Collections.emptyList();
		}  catch (SQLException e) {
			LOGGER.error("[RiscossioneAmbienteDAOImpl::searchRiscossione] Errore nell'esecuzione della query ", e);
            throw new Exception(e);
		} catch (DataAccessException e) {
			LOGGER.error("[RiscossioneAmbienteDAOImpl::searchRiscossione] Errore nell'accesso ai dati ", e);
            throw new Exception(e);
		} catch (ParseException e) {
			LOGGER.error("[RiscossioneAmbienteDAOImpl::searchRiscossione] Errore parse  ", e);
            throw new Exception(e);
		} catch (JsonParseException e) {
			LOGGER.error("[RiscossioneAmbienteDAOImpl::searchRiscossione] Errore Json Parse  ", e);
            throw new Exception(e);
		} catch (JsonMappingException e) {
			LOGGER.error("[RiscossioneAmbienteDAOImpl::searchRiscossione] Errore Json Mapping  ", e);
            throw new Exception(e);
		} catch (IOException e) {
			LOGGER.error("[RiscossioneAmbienteDAOImpl::searchRiscossione] Errore generale ", e);
            throw new Exception(e);
		} finally {
			LOGGER.debug("[RiscossioneAmbienteDAOImpl::searchRiscossione] END");
		}
	}


	private void validateRiscossione(RiscossioneDTO dto) throws SQLException, SystemException  {
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::validateRiscossione] BEGIN");
...
//			}
		...
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::validateRiscossione] END");
	}

	@Transactional
	public Long updateRiscossione(RiscossioneDTO dto) throws BusinessException, Exception {
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::updateRiscossione] BEGIN");
		...
		try {
			...
			try {
				...
				LOGGER.error("[RiscossioneAmbienteDAOImpl::updateRiscossione] ERROR : " +e);
			}
...
//            String uid = generateGestUID(dto.getCfAccredito() + dto.getDesEmailAccredito() + now);
			...
			Long idSoggettoOld = template.query(QUERY_SELECT_ID_SOGGETTO_BY_ID_RISCOSSIONE, params, new ResultSetExtractor<Long>(){
			    @Override
			...
			if (!idSoggettoOld.equals(dto.getSoggetto().getIdSoggetto())) {
				...
				String dataInizioTitolaritaString  = template.query(QUERY_SELECT_DATA_INIZIO_TITOLARITA_BY_ID_RISCOSSIONE, params, new ResultSetExtractor<String>(){
				    @Override
...
//			cancello tutti i provvedimenti prima dell'insert
			...
			LOGGER.error("[RiscossioneAmbienteDAOImpl::updateRiscossione] Errore nell'accesso ai dati", e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			throw new Exception();
		} finally {
			LOGGER.debug("[RiscossioneAmbienteDAOImpl::updateRiscossione] END");
		}
	}
	...
	private String addJoinQueryFromDto(RiscossioneSearchDTO dto,String modalitaRicerca , String query) {
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::addJoinQueryFromDto] BEGIN");
		...
		if(dto.getProvincia() != null) {
			if(dto.getComuneDiResidenza() != null) {
				querySearch = querySearch +	 "  left join risca_d_provincia rdp  on rdc.id_provincia  =  rdp.id_provincia ";
			}else {
		...
		if(dto.getCodiceAvviso() != null || dto.getNap() != null) {
			querySearch = querySearch + " left join RISCA_T_STATO_DEBITORIO rtsd   on rtsd.id_riscossione  = rtr.id_riscossione   "
					+ " left join RISCA_T_IUV rti   on rti.nap  = rtsd.nap  ";
		}
		if(dto.getCanone() != null ) {
			querySearch = querySearch + " inner join risca_r_rata_sd rrrs ON rtsd.id_stato_debitorio = rrrs.id_stato_debitorio ";
		}
		if(modalitaRicerca != null) {
			if(dto.getDatiTecnici() != null || dto.getAnnoCanone() != null) {
				querySearch = querySearch + " inner join risca_r_annualita_sd rras   on rtsd.id_stato_debitorio = rras.id_stato_debitorio  ";

			}
		}
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::addJoinQueryFromDto] END");
		return querySearch;
	}
	...
	public static class RiscossioneSearchRowMapper implements RowMapper<RiscossioneSearchResultDTO> {
		...
		public RiscossioneSearchResultDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			try {
				...
				LOGGER.error(
						"[RiscossioneAmbienteDAOImpl::searchRiscossione::mapRow::populateBean] Errore nella popolazione bean",
						e);
			}
			return bean;
		}
		...
		private void populateBean(ResultSet rs, RiscossioneSearchResultDTO bean)
			...
			if (rs.getString("json_dt") != null) {
				DatiTecniciAmbienteDTO datiTecniciAmbiente = Utils.creaDatiTecniciFromJsonDt(rs.getString("json_dt"), "dati_tecnici");
				if(datiTecniciAmbiente.getDatiGenerali() != null) {
	...
	public List<RiscossioneRecapitoDTO> checkRiscossioneRecapiti(Long idRiscossione) {
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::checkRiscossioneRecapiti] BEGIN");
		try {
			Map<String, Object> map = new HashMap<>();
			...
			LOGGER.error("[RiscossioneAmbienteDAOImpl::checkRiscossioneRecapiti] Errore nell'esecuzione della query",
					e);
			return null;
		} catch (DataAccessException e) {
			LOGGER.error("[RiscossioneAmbienteDAOImpl::checkRiscossioneRecapiti] Errore nell'accesso ai dati", e);
			return null;
		} finally {
			LOGGER.debug("[RiscossioneAmbienteDAOImpl::checkRiscossioneRecapiti] END");
		}
	}
	...
	public Integer checkNumPratica(String numPratica) {
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::checkNumPratica] BEGIN");
		try {
			Map<String, Object> map = new HashMap<>();
			...
			try {
				tipiRiscossione = tipoRiscossioneDaoImpl
						.loadTipiRiscossioneByIdAmbitoAndDateValidita(idAmbito, null, null);
			} catch (ParseException e) {
				LOGGER.error("[RiscossioneAmbienteDAOImpl::checkCodRiscossione] Errore nell'esecuzione della query", e);
			...
			LOGGER.error("[RiscossioneAmbienteDAOImpl::checkNumPratica] Errore nell'esecuzione della query", e);
			return null;
		} catch (DataAccessException e) {
			LOGGER.error("[RiscossioneAmbienteDAOImpl::checkNumPratica] Errore nell'accesso ai dati", e);
			return null;
		} finally {
			LOGGER.debug("[RiscossioneAmbienteDAOImpl::checkNumPratica] END");
		}
	}
	public Integer checkCodRiscossione(String codRiscossione) {
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::checkCodRiscossione] BEGIN");
		try {
			Map<String, Object> map = new HashMap<>();
			...
			LOGGER.error("[RiscossioneAmbienteDAOImpl::checkCodRiscossione] Errore nell'accesso ai dati", e);
			return null;
		} finally {
			LOGGER.debug("[RiscossioneAmbienteDAOImpl::checkCodRiscossione] END");
		}
	}
...
//				crea query per lo SD 
	...
	public Integer countRiscossioniDelegati(RiscossioneSearchDTO dto, long idAmbito, String modalitaRicerca) throws Exception {
		String methodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		LOGGER.debug(getClassFunctionBeginInfo(className, methodName));
		try {
			Map<String, Object> map = new HashMap<>();
	...
	public Long updateRiscossioneStatoPratica(RiscossioneDTO dto) throws BusinessException, Exception {
		LOGGER.debug("[RiscossioneAmbienteDAOImpl::updateRiscossioneStatoPratica] BEGIN");
		Long idRiscossione = dto.getIdRiscossione();
		try {
			...
			params = getParameterValue(map);
		    template.update(getQuery(QUERY_UPDATE_RISCOSSIONE_STATO_PRATICA_SCADUTA, null, null), params);		    
			template.update(getQuery(QUERY_UPDATE_RISCOSSIONE_STATO_PRATICA_SOSPESA, null, null), params);
			template.update(getQuery(QUERY_UPDATE_RISCOSSIONE_STATO_PRATICA_ATTIVA, null, null), params);

			

		} catch (DataAccessException e) {
			LOGGER.error("[RiscossioneAmbienteDAOImpl::updateRiscossioneStatoPratica] Errore nell'accesso ai dati", e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			throw new Exception();
		} finally {
			LOGGER.debug("[RiscossioneAmbienteDAOImpl::updateRiscossioneStatoPratica] END");
		}
		return idRiscossione;
	}
