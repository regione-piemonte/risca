...
public class RataSdTributiDAOImpl extends RiscaBeSrvGenericDAO<RataSdDTO>  {
	...
	public List<Integer> saveNRataSd(TipoDilazioneDTO tipoDilazione, StatoDebitorioExtendedDTO statoDebitorio) throws DAOException, BusinessException, SystemException {
		List<RataSdDTO> rate = new ArrayList<RataSdDTO>();
		...
		//QUERY PER RECUPERARE LA DATA TRAMITE ID STATO DEBITORIO
			...
			RataSdDTO rataNSd = new RataSdDTO();
					...
					if(rataSdPadre.getDataScadenzaPagamento() != null) {
						if(i == 0) {
							...
							try {
								...
								LOGGER.error("[RataSdTributiDAOImpl::saveNRataSd] Errore parse data prima rata ", e);
							}
							  ...
							  try {
								...
								LOGGER.error("[RataSdTributiDAOImpl::saveNRataSd] Errore parse data rate successive ", e);
							}
			...
			LOGGER.error("[RataSdTributiDAOImpl::saveNRataSd] Errore nell'accesso ai dati",
					e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			throw new DAOException(Constants.ERRORE_GENERICO);
		} catch (SQLException e) {
			LOGGER.error(
					"[RataSdTributiDAOImpl::saveNRataSd] Errore nell'esecuzione della query",
					e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			throw new DAOException(Constants.ERRORE_GENERICO);
		}
...
//				map.put("canoneDovuto", canoneDovutoRateizzato);
					...
					if(rataSdPadre.getDataScadenzaPagamento() != null) {
						if(i == 0) {
							...
							try {
								...
								LOGGER.error("[RataSdTributiDAOImpl::updateNRataSd] Errore parse data prima rata ", e);
							}
							  ...
							  try {
								...
								LOGGER.error("[RataSdTributiDAOImpl::updateNRataSd] Errore parse data rate successive ", e);
							}
			...
			LOGGER.error("[RataSdTributiDAOImpl::updateNRataSd] Errore nell'accesso ai dati",
					e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			throw new DAOException(Constants.ERRORE_GENERICO);
		} catch (SQLException e) {
			LOGGER.error(
					"[RataSdTributiDAOImpl::updateNRataSd] Errore nell'esecuzione della query",
					e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			throw new DAOException(Constants.ERRORE_GENERICO);
		}
	...
	public void deleteNRataSd(Long idStatoDebitorio) throws DAOException{
		LOGGER.debug("[RataSdTributiDAOImpl::deleteNRataSd] BEGIN");
		...
		try {
			...
			LOGGER.error("[RataSdTributiDAOImpl::deleteNRataSd] ERROR : ", e);
			throw new DAOException(Constants.ERRORE_GENERICO);
		} finally {
			LOGGER.debug("[RataSdTributiDAOImpl::deleteNRataSd] END");
		}
