...
public class TipoRicercaPagamentiApiServiceHelper  extends AbstractServiceHelper {
	
	private final String className = this.getClass().getSimpleName();
	
	private static final String TIPI_RICERCA_PAGAMENTI = "/tipi-ricerca-pagamenti"; 
	private static final String RICERCA_PAGAMENTI = "/ricerca_pagamenti"; 

	
    public TipoRicercaPagamentiApiServiceHelper(String hostname, String endpointBase) {
        this.hostname = hostname;
        this.endpointBase = hostname + endpointBase;
    }

	public List<TipoRicercaPagamentoDTO> loadAllTipiRicercaPagamenti(HttpHeaders httpHeaders,
			HttpServletRequest httpRequest) throws GenericException {
        String methodName = Thread.currentThread().getStackTrace()[1].getMethodName();
        LOGGER.debug(getClassFunctionBeginInfo(className, methodName));
        String targetUrl = Utils.buildTargetUrl(this.endpointBase+TIPI_RICERCA_PAGAMENTI);
        List<TipoRicercaPagamentoDTO>  result = new ArrayList<>();
        try {
            ...
            if  (resp.getStatus() == 200) {
                GenericType<List<TipoRicercaPagamentoDTO>> dtoType = new GenericType<>() {};
	...
	public Response ricercaPagamenti(RicercaPagamentoDTO ricercaPagamentoDTO, String fruitore, Integer offset,
			Integer limit, String sort, HttpHeaders httpHeaders, HttpServletRequest httpRequest) throws GenericException {
		 String methodName = Thread.currentThread().getStackTrace()[1].getMethodName();
	        LOGGER.debug(getClassFunctionBeginInfo(className, methodName));
	        String targetUrl = Utils.buildTargetUrl(this.endpointBase+RICERCA_PAGAMENTI,Constants.FRUITORE, fruitore,"offset",offset,
	        		"limit",limit,"sort",sort);
	        try {
	             MultivaluedMap<String, Object> map = getMultivaluedMapFromHttpHeaders(httpHeaders, httpRequest);
	             Entity<RicercaPagamentoDTO> entity = Entity.json(ricercaPagamentoDTO);
