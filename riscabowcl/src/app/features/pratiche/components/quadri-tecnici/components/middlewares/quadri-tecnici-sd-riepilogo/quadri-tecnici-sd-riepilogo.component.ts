...
import { DTAmbienteSDRiepilgoComponent } from '../../ambito-ambiente/version-20211001/dt-ambiente-sd-riepilogo/dt-ambiente-sd-riepilogo.component';
import { DTTributiSDRiepilogoComponent } from '../../ambito-tributi/version-20220101/dt-tributi-sd-riepilogo/dt-tributi-sd-riepilogo.component';
import { DatiTecniciKeys } from '../../core/dati-tecnici-pratica/utilities/dati-tecnici-pratica.enums';
import { QuadriTecniciSDRiepilogoConsts } from './utilities/quadri-tecnici-sd-riepilogo.consts';
 ...
 */
@Component({
  selector: 'quadri-tecnici-sd-riepilogo',
  templateUrl: './quadri-tecnici-sd-riepilogo.component.html',
  styleUrls: ['./quadri-tecnici-sd-riepilogo.component.scss'],
})
export class QuadriTecniciSDRiepilogoComponent
  extends RiscaFormParentAndChildComponent<PraticaDTVo>
  implements OnInit
{
  /** Oggetto contenente le costanti utilizzate nel componente. */
  QTSDR_C = QuadriTecniciSDRiepilogoConsts;
  ...
  /** StatoDebitorioVo per la pre-valorizzazione del componente dei dati tecnici (usato principalmente per il dettaglio/modifica). */
  ...
  /** Oggetto contenente una coppia di chiavi valore per il caricamento dei dati tecnici dello stato debitorio, scheda riepilogo. */
  private _dtSDRiepilogoComponents: DynamicObjAny;
  ...
  private setupComponentiDatiTecnici() {
     ...
     * ####################################
     */
    // COMPONENTI PER: STATO DEBITORIO - RIEPILOGO
    this._dtSDRiepilogoComponents = {
      DatiTecniciAmbiente: DTAmbienteSDRiepilgoComponent,
      DatiTecniciTributi: DTTributiSDRiepilogoComponent,
    };
  }
  ...
  private setupFormSubmitHandlerService() {
    ...
    const parent = this.QTSDR_C.FORM_KEY_PARENT;
    // Definisco le chiavi per i figli
    const children = [this.QTSDR_C.FORM_KEY_CHILD];
  ...
  private initDatiTecnici() {
    ...
    let inputs = new DTRiepilogoSDConfig();
    inputs.parentFormKey = this.QTSDR_C.FORM_KEY_PARENT;
    inputs.childFormKey = this.QTSDR_C.FORM_KEY_CHILD;
    inputs.modalita = this.modalita;
    inputs.praticaDT = this.praticaDT;
    inputs.disableUserInputs = this.AEA_DTDisabled;
    ...
    const options = {
      providers: [{ provide: DT_RIEPILOGO_SD_CONFIG, useValue: inputs }],
  ...
  formsValid(formsValid: IRiscaFormTreeParent) {
    ...
    const datiTecnici = mapData.get(this.QTSDR_C.FORM_KEY_CHILD);
  ...
  private get _datiTecniciComponent(): DynamicObjAny {
    ...
    return this._dtSDRiepilogoComponents;
  }
}
