...
import {
  ...
  typeaheadComuneFormatter,
  desSelectDataFineVal,
} from '../risca-utilities/risca-utilities.functions';
...
export class RiscaRecapitoService {
  ...
  setupFormInputs() {
    ...
    const typeaheadSearch = (v: string): Observable<ComuneVo[]> => {
    ...
    const typeaheadMap = (c: ComuneVo): string => {
      // Richiamo la funzione di comodo per la formattazione
      let comune: string = typeaheadComuneFormatter(c);

      // Verifico se il comune non è più attivo
      if (!isDataValiditaAttiva(c)) {
        // Definisco la variabile con il formato della data
        const format = this.C_C.DATE_FORMAT_VIEW;
        // Estraggo la data di fine validità
        const fineVal = moment(c.data_fine_validita).format(format);
        // Aggiungo la data di fine validità come descrizione
        return desSelectDataFineVal(fineVal, comune);
      }

      // Ritorno la descrizione del comune
      return comune;
    };
    ...
    // Aggiungo al type ahead del comune la configurazione per la gestione della data validita
    comuneConfig.data.dataValidita = true;
